...
@Gtk.Template(resource_path='/io/github/lo2dev/Mutiny/ui/chat-message.ui')
class ChatMessage(Gtk.Box):
    ...
    def __init__(self, message_data, cascade=False, client_user=None, **kwargs):
        ...
        if 'system' in message_data:
            if message_data['system']['type'] == 'user_left':
                self.avatar_menu.props.visible = False
                self.username.props.label = "Broski left lmao"
                self.username.add_css_class("dim-label")
            elif message_data['system']['type'] == 'user_joined':
                self.avatar_menu.props.visible = False
                self.username.props.label = "Broski joined lmao"
                self.username.add_css_class("dim-label")
        else:
            if cascade:
                self.avatar_menu.props.visible = False
